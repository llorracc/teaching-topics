# Orchestrator: Notebook Editing

**Phase:** Skills → Artifacts  
**Time:** ~10 minutes

## Purpose

Make your first edit to the ballpark notebook: adding yourself as coauthor.

---

## Prerequisites

- Environment working (from [Environment Setup](01-environment-setup.md))
- Your ballpark notebook opens in Cursor

---

## Ask Cursor Throughout

For any Git questions:
> "I'm trying to [describe what you want to do]. What Git command should I use?"

---

## Workflow sequence

### 1. [Add yourself as coauthor](../workflows/add-coauthor-to-notebook.md)

Add your name to the notebook; save.

**Key points:**
- Find the authors cell (usually near the top)
- Add yourself following the existing format
- Save the notebook (Cmd+S / Ctrl+S)

**Deliverable:** Notebook shows you as a coauthor

---

## Checkpoint

Before proceeding, verify:
- [ ] Your name appears in the authors section
- [ ] Notebook is saved
- [ ] `git status` shows the notebook as modified

---

## Assignment

This orchestrator prepares you for:
- [Assignment 050: Add Coauthor + PR](../assignments/add-coauthor-and-pr.md)

---

## Next orchestrator

[MyST Conversion](05-myst-conversion.md) (optional for now)

Or skip to:
[Contribution](06-contribution.md) (to create your PR)
