...
Run:
```bash
ls ~/GitHub/<your-username>/ballpark/README.md
```

**Expected:** Shows the file path (no error)

**If "No such file":** Fork then clone the repo:

1. Fork at https://github.com/econ-ark/ballpark (click **Fork**)
2. Clone your fork:
```bash
mkdir -p ~/GitHub/<your-username>
cd ~/GitHub/<your-username>
...
# If you have an upstream remote (recommended), pull from upstream:
git pull upstream master

# Otherwise (if you only have origin), pull from your fork:
...
Run:
```bash
ls ~/GitHub/<your-username>/ballpark/.venv-*/
```

**Expected:** Shows a directory like `.venv-darwin-arm64/` or `.venv-linux-x86_64/` (depends on your system)

**If empty or missing:** The environment needs to be created. Ask Cursor:
> "How do I install the environment for this repo? I think I should run scripts/setup_env.sh."
